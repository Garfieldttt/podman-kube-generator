...
from .quadlet import generate_quadlet, generate_prune_units
...
# App-seitige DB-Verbindungs-Vars pro DB-Typ
_APP_DB_VARS = {
    'mysql': {
        'host': ['MYSQL_HOST', 'MARIADB_HOST', 'DB_HOST', 'DATABASE_HOST',
                 'WORDPRESS_DB_HOST', 'NEXTCLOUD_DB_HOST'],
        'user': ['MYSQL_USER', 'MARIADB_USER', 'DB_USER', 'DATABASE_USER',
                 'WORDPRESS_DB_USER', 'NEXTCLOUD_DB_USER'],
        'pass': ['MYSQL_PASSWORD', 'MARIADB_PASSWORD', 'DB_PASSWORD', 'DATABASE_PASSWORD',
                 'WORDPRESS_DB_PASSWORD', 'NEXTCLOUD_DB_PASSWORD'],
        'db':   ['MYSQL_DATABASE', 'MARIADB_DATABASE', 'DB_NAME', 'DATABASE_NAME',
                 'WORDPRESS_DB_NAME', 'NEXTCLOUD_DB_NAME'],
    },
    'postgres': {
        'host': ['POSTGRES_HOST', 'PGHOST', 'DB_HOST', 'DATABASE_HOST'],
        'user': ['POSTGRES_USER', 'PGUSER', 'DB_USER', 'DATABASE_USER'],
        'pass': ['POSTGRES_PASSWORD', 'PGPASSWORD', 'DB_PASSWORD', 'DATABASE_PASSWORD'],
        'db':   ['POSTGRES_DB', 'PGDATABASE', 'DB_NAME', 'DATABASE_NAME'],
    },
}


def _classify_db_connections(form_data):
    """
    Identifiziert DB-Container und welche App-Container sich lokal verbinden.
    Gibt zurück: {db_type: {'container': name, 'primary': {...}, 'apps': [...]}}
    Nur mysql und postgres — Redis/MongoDB brauchen kein SQL init script.
    """
    containers = form_data.get('containers', [])
    result = {}
    container_names = {c.get('name', '').strip() for c in containers}

    for c in containers:
        base = _img_base(c.get('image', ''))
        if base not in _DB_IMAGES:
            continue
        env = _parse_env_str(c.get('env', ''))
        if 'mariadb' in base or 'mysql' in base:
            db_type = 'mysql'
            primary = {
                'user':       env.get('MYSQL_USER') or env.get('MARIADB_USER', ''),
                'pass':       env.get('MYSQL_PASSWORD') or env.get('MARIADB_PASSWORD', ''),
        ...
        elif 'postgres' in base:
            db_type = 'postgres'
            primary = {
                'user':       env.get('POSTGRES_USER', 'postgres'),
                'pass':       env.get('POSTGRES_PASSWORD', ''),
                'db':         env.get('POSTGRES_DB') or env.get('POSTGRES_USER', 'postgres'),
                'image_base': base,
            }
        else:
            continue
        if db_type not in result:
            result[db_type] = {'container': c['name'], 'primary': primary, 'apps': []}
    ...
    for c in containers:
        base = _img_base(c.get('image', ''))
        if base in _DB_IMAGES:
            continue
        env = _parse_env_str(c.get('env', ''))
        for db_type, var_map in _APP_DB_VARS.items():
            if db_type not in result:
                continue
            host_val = next((env[v] for v in var_map['host'] if v in env), None)
            if host_val is not None:
                host_bare = host_val.split(':')[0]
                if host_bare and host_bare not in ('127.0.0.1', 'localhost') \
                        and host_bare not in container_names:
                    continue
            user_val = next((env[v] for v in var_map['user'] if v in env), '')
            pass_val = next((env[v] for v in var_map['pass'] if v in env), '')
            db_val   = next((env[v] for v in var_map['db']   if v in env), '')
            if user_val or db_val:
                result[db_type]['apps'].append({
                    'name': c['name'],
                    'user': user_val,
                    'pass': pass_val,
                    'db':   db_val,
...
def _build_mysql_init_sql(primary, extra_apps):
    """SQL für extra User/DBs in MariaDB/MySQL. Gibt (sql, [app_names]) zurück."""
    lines = ['-- Auto-generated by podman-kube-gen: extra databases for multi-app pod', '']
    added = []
    for app in extra_apps:
        user = (app.get('user') or '').strip()
        pw   = (app.get('pass') or '').strip()
        db   = (app.get('db')   or '').strip() or user
        ...
        pw_esc   = pw.replace("'", "''")
        if db_esc:
            lines.append(f"CREATE DATABASE IF NOT EXISTS `{db_esc}`;")
        if user_esc:
            lines.append(f"CREATE USER IF NOT EXISTS '{user_esc}'@'%' IDENTIFIED BY '{pw_esc}';")
            if db_esc:
                lines.append(f"GRANT ALL PRIVILEGES ON `{db_esc}`.* TO '{user_esc}'@'%';")
        lines.append('')
...
def _build_postgres_init_sql(primary, extra_apps):
    """Idempotentes SQL für extra Roles/Databases in PostgreSQL. Gibt (sql, [app_names]) zurück."""
    lines = ['-- Auto-generated by podman-kube-gen: extra databases for multi-app pod', '']
    added = []
    for app in extra_apps:
        user = (app.get('user') or '').strip()
        ...
        if user:
            lines.append(
                ...
                f"    CREATE ROLE \"{user}\" WITH LOGIN PASSWORD '{pw_esc}';\n"
                f"  END IF;\n"
                f"END $$;"
            )
        if db:
            ...
            lines.append(
                f"SELECT 'CREATE DATABASE \"{db}\"{owner_clause}'"
                f" WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '{db}')\\gexec"
            )
            if user:
                lines.append(f'GRANT ALL PRIVILEGES ON DATABASE "{db}" TO "{user}";')
        lines.append('')
        added.append(app['name'])
    if not added:
        return '', []
...
def _inject_db_init(form_data):
    """
    Pre-Processor: erkennt Multi-App-DB-Szenario und injiziert automatisch
    einen busybox Init-Container mit SQL-Init-Script via /docker-entrypoint-initdb.d.
    Modifiziert form_data in-place. Gibt Info-Notices zurück.
    """
    import base64 as _b64
    connections = _classify_db_connections(form_data)
    notices = []

    for db_type, info in connections.items():
        primary = info['primary']
        extra_apps = [
            a for a in info['apps']
            if not (a.get('user') == primary.get('user') and a.get('db') == primary.get('db'))
            and (a.get('user') or a.get('db'))
        ]
        if not extra_apps:
            continue

        if db_type == 'mysql':
            sql, added = _build_mysql_init_sql(primary, extra_apps)
        else:
            sql, added = _build_postgres_init_sql(primary, extra_apps)
        if not sql or not added:
            continue

        pod_name = re.sub(r'[^a-z0-9-]', '', (form_data.get('pod_name') or 'pod').strip().lower().replace(' ', '-')).strip('-') or 'pod'
        vol_name = f'{pod_name}-db-init-{db_type}'
        db_cname = info['container']
        ic_name  = f'db-init-{db_type}'

        existing_ic_names = {ic.get('name', '') for ic in form_data.get('init_containers', [])}
        if ic_name in existing_ic_names:
            continue
        ...
        init_container = {
            ...
            'command':    'sh',
            'args':       f'-c "mkdir -p /db-init && echo {sql_b64} | base64 -d > /db-init/01-init.sql"',
            'volumes':    f'{vol_name}:/db-init',
            'env':        '',
            'run_always': False,
        }
        form_data.setdefault('init_containers', []).insert(0, init_container)

        for c in form_data.get('containers', []):
            if c.get('name') == db_cname:
                existing_vols = (c.get('volumes') or '').strip()
                new_mount = f'{vol_name}:/docker-entrypoint-initdb.d'
                if new_mount not in existing_vols:
                    c['volumes'] = (existing_vols + '\n' + new_mount).strip()
                break

        apps_str = ', '.join(f'<code>{n}</code>' for n in added)
        notices.append({
            'level': 'info',
            'msg': (
...
def validate_form_data(form_data):
    ...
    for c in app_cs:
        ...
        if pg_host is not None and not has_postgres_container:
            ...
            if pg_host_bare in ('127.0.0.1', 'localhost', ''):
                warnings.append({
                    'level': 'error',
                    'msg': (
        ...
        if my_host is not None and not has_mysql_container:
            ...
            if my_host_bare in ('127.0.0.1', 'localhost', ''):
                warnings.append({
                    'level': 'error',
                    'msg': (
...
def image_inspect(request):
    ...
    if is_dockerhub:
        ...
        tasks['tags']  = ex.submit(get_tags, namespace, name)
    ...
    # Docker Hub URL (public, no login required)
    if namespace == 'library':
        scout_url = f"https://hub.docker.com/_/{name}/tags?name={tag}"
    else:
        scout_url = f"https://hub.docker.com/r/{namespace}/{name}/tags?name={tag}"
